...
class RelTRController:
    def __init__(self):
        self.ckpt = args.Checkpoint.ckpt_sgg
    ...
    def rescale_bboxes(out_bbox, size):
        img_w, img_h = size
        b = RelTRController.box_cxcywh_to_xyxy(out_bbox)
    ...
    def sgg_controller(self):
        try:
            ...
            with torch.no_grad():
                ...
                self.draw_single_object_dect(im, indices, keep_queries, sub_bboxes_scaled, obj_bboxes_scaled, out_res_obj)
                self.draw_per_detail_triplet(im, indices, keep_queries, sub_bboxes_scaled, obj_bboxes_scaled, out_res_obj)
                self.draw_detail_triplet(im, indices, keep_queries, sub_bboxes_scaled, obj_bboxes_scaled, probas_sub, probas, probas_obj, out_triplet)
                self.draw_sg(sList,oList,rList, out_res_grp)
    ...
    def upload_image(self, filename):
        if('object+' in filename):
            return send_from_directory(self.dir_upload + filename.replace('.jpg', '').replace('object+', ''), filename)
        if('graph+' in filename):
            return send_from_directory(self.dir_upload + filename.replace('.jpg', '').replace('graph+', ''), filename)
        if('triplet+' in filename):
            return send_from_directory(self.dir_upload + filename.replace('.jpg', '').replace('triplet+', ''), filename)
