...
class CreateData(Dataset):
    def __init__(self, img_folder, ann_file, rel_file, transforms, tokenizer, image_set):
        with open(ann_file, 'r') as f:
            self.data = json.load(f)

        with open(rel_file, 'r') as r:
            all_rel = json.load(rel_file)

        self.rel = all_rel[image_set]

        self.rel_categories = all_rel['rel_categories']
...
def build_data(image_set, annotation_file, image_folder, tokenizer):
    dataset = CreateData(image_folder, annotation_file, transforms=make_coco_transforms(image_set), tokenizer = tokenizer, image_set = image_set)
    return  dataset