...
def get_set():
    ...
    Image2GraphEmbedding = entity.Image2GraphEmbeddingV2  
    image_set = []
    triplet_set = []
    try:
        results = db.session.query(
            Image2GraphEmbedding.image_name, Image2GraphEmbedding.triplets
        ).filter(
            Image2GraphEmbedding.image_name.in_(imgs.tolist())
        ).all()
        for row in results:
            image_set.append(row.image_name)
            triplet_set.append(set(parse_postgres_array(row.triplets)))
